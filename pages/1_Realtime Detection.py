...
def video_frame_callback(frame: av.VideoFrame) -> av.VideoFrame:
	...
	# Read dynamic inference settings from session state so slider changes take effect live
	conf_thres = float(st.session_state.get("rt_conf", 0.5))
	iou_thres = float(st.session_state.get("rt_iou", 0.7))
	imgsz = int(st.session_state.get("rt_imgsz", 640))
	enable_tta = bool(st.session_state.get("rt_tta", False))
...
st.markdown("#### Camera")
preview_only = st.toggle("Preview camera (no detection)", value=False)
...
# Inference controls
st.session_state["rt_conf"] = st.slider("Confidence", min_value=0.0, max_value=1.0, value=0.5, step=0.05)
st.session_state["rt_iou"] = st.slider("IOU (NMS)", min_value=0.1, max_value=0.95, value=float(st.session_state.get("rt_iou", 0.7)), step=0.05)
st.session_state["rt_imgsz"] = st.select_slider("Image size", options=[640, 960, 1280], value=int(st.session_state.get("rt_imgsz", 640)))
st.session_state["rt_tta"] = st.checkbox("Enable TTA (Test-Time Augmentation)", value=bool(st.session_state.get("rt_tta", False)))
